...
for(trim in tpoints){
  ...
  #Make race into dummy variables 
  #anobig <- anobig %>%
    #mutate("Race_dv" = Race)
  #anobig$Race_dv <- gsub("African American", "0", anobig$Race_dv)
  #anobig$Race_dv <- gsub("Caucasian", "1", anobig$Race_dv)
  #anobig$Race_dv <- gsub("Asian", "2", anobig$Race_dv)
  ...
  for(ite in 1:length(folds)){
    ...
    train=data.frame(G=factor(ano_tr$G), BMI=as.numeric(ano_tr$BMI), Age=as.numeric(ano_tr$Age), Nulli=as.numeric(ano_tr$Nulliparity),t(esetn_tr[sel,]))
    test=data.frame(G=factor(ano_t$G), BMI=as.numeric(ano_t$BMI), Age=as.numeric(ano_t$Age), Nulli=as.numeric(ano_t$Nulliparity),t(esetn_t[sel,]))
    ...
    if(modtype=="EN"){
      ...
      p_recipe <- recipe(G ~ ., data = train) %>% 
        step_zv(all_numeric(), -all_outcomes()) #Removes all numeric predictors with zero variance
        #step_zv(all_numeric(), -all_outcomes(), -all_factor())
  ...
  pdf(paste("../Prediction/Prediction_all_proteins_clinical/ROC_wilcoxon_",trim,"_",modtype,"_clinical.pdf",sep=""))
  ...
  #Boxplot
  a_mod <- a
  ano_mod <- ano %>% select(Cohort_G, Cohort)
  ...
  ggsave(filename= paste("Boxplots_all_proteins_wilcoxon_",trim,"_",modtype,"_clinical.png",sep=""),
         plot = b1,
        device = "png",
         path = "../Prediction/Prediction_all_proteins_clinical",
         width = 20,
        height = 10,
         units = "cm")
  ...
  save(pile,freq,file=paste("../Prediction/Prediction_all_proteins_clinical/Predict_all_proteins_",trim,"",modtype,"_clinical.RData",sep=""))
