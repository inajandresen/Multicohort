...
for(trim in tpoints){
  ...
  folds=list()
  folds[["Fold01"]]<-which(anou$Cohort=="Detroit")
  folds[["Fold02"]]<-which(anou$Cohort=="Oslo")
  folds[["Fold03"]]<-which(anou$Cohort=="Stanford")
  ...
  pdf(paste("../Prediction/Predict_2vs1_all_proteins/ROC_2vs1_all_proteins_",trim,"_",modtype,".pdf",sep=""))
  
  RC_dt=roc(response=a[grep("Detroit", rownames(a)),1],predictor=a[grep("Detroit", rownames(a)),2],direction="<")
  plot(1-RC_dt$specificities,RC_dt$sensitivities,col="#F8766D",type="l",lwd=2,xlab="False positive rate",
         ylab="Sensitivity")
  abline(0,1)
  ...
  legend <- c(paste("Detroit AUC=",aucsrf_dt[2],"(",aucsrf_dt[1],"-",aucsrf_dt[3],")",sep=""),
              paste("Stanford AUC=",aucsrf_std[2],"(",aucsrf_std[1],"-",aucsrf_std[3],")",sep=""),
              paste("Oslo AUC=",aucsrf_osl[2],"(",aucsrf_osl[1],"-",aucsrf_osl[3],")",sep=""))
  
  legend("bottomright", legend = legend, col = c("#F8766D", "#619CFF" ,"#00BA38"), lwd=2)
  title(paste("2vs1",modtype,trim))            
  ...
  b1 <- ggplot(ano_mod, aes(x=Cohort_G, y=out, fill = Cohort)) + 
    ...
    ggtitle(paste("2vs1",modtype,trim, sep=" ")) +
    theme(legend.title = element_blank())
  
  ggsave(filename= paste("Boxplots_2vs1_all_proteins_",trim,"_",modtype,".png",sep=""),
         plot = b1,
         device = "png",
         path = "../Prediction/Predict_2vs1_all_proteins",
         width = 20,
         height = 10,
         units = "cm")
  ...
  save(pile,freq,file=paste("../Prediction/Predict_2vs1_all_proteins/Predict_2vs1_all_proteins_",trim,"_",modtype,".RData",sep=""))
